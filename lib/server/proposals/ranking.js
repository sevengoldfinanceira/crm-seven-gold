...
function rankProposals(allOptions, params) {
  ...
  allOptions.forEach((option) => {
    ...
    // If useHalfInst is enabled, compare against 50% of final installment.
    // Otherwise, compare against 100% (full/integral) of final installment.
  ...
  if (priority === 'MAIOR_CREDITO') {
    validProposals.sort((a, b) => {
      ...
      // 2. Maior tabela de faixa (AUTOCON PRIME 1 > PRIME 2 > PRIME 3 > PRIME 4)
      ...
      if (aPrimeMatch && bPrimeMatch) {
        ...
        if (aNum !== bNum) return aNum - bNum; // PRIME 1 < PRIME 2 in number, so PRIME 1 comes first
      }
