...
function rankProposals(allOptions, params) {
  ...
  const priority = (params.ranking_priority || 'MAIOR_CREDITO').toUpperCase();
  // use_half_installment=true → use 50% (meia parcela, default behavior).
  // use_half_installment=false → use 100% (parcela integral, when checkbox is ticked).
  const useHalfInst = params.use_half_installment === true || params.use_half_installment === 'true' || params.half_installment === true || params.half_installment === 'true';

  const validProposals = [];
  const nearMatches = [];

  allOptions.forEach((option) => {
    // All active imported proposal options are eligible for simulation
    ...
    // Default (useHalfInst = true): compare against 50% of final installment (Meia Parcela).
    // When user chose integral (useHalfInst = false): compare against 100% (full installment).
    const instToCompare = useHalfInst ? (finalInst * 0.5) : finalInst;

    // Calculate budget excesses
    const firstExcess = (maxFirstInst < Infinity && firstInst > maxFirstInst) ? (firstInst - maxFirstInst) : 0;
    const instExcess = (maxInst < Infinity && instToCompare > maxInst) ? (instToCompare - maxInst) : 0;
    ...
    const NEAR_MAX_INST_EXCESS = 500;    // max R$500 over installment limit
    const NEAR_MAX_FIRST_EXCESS = 5000;  // max R$5000 over entrada limit

    if (isCreditValid && instExcess === 0 && firstExcess === 0) {
      ...
      isCreditValid &&
      instExcess <= NEAR_MAX_INST_EXCESS &&
      firstExcess <= NEAR_MAX_FIRST_EXCESS &&
      (instExcess > 0 || firstExcess > 0)
    ) {
      // Near match: exceeded limits slightly but within tolerance
      const excessMessages = [];
      if (firstExcess > 0) excessMessages.push(`Ultrapassa a entrada em R$ ${firstExcess.toFixed(2).replace('.', ',')}`);
      if (instExcess > 0) excessMessages.push(`Ultrapassa a parcela em R$ ${instExcess.toFixed(2).replace('.', ',')}`);

      nearMatches.push({
        ...option,
        credit_diff: hasDesiredCredit ? Math.abs(credit - desiredCredit) : 0,
        excess_amount: instExcess + firstExcess,
  ...
  if (priority === 'MAIOR_CREDITO') {
    validProposals.sort((a, b) => {
      ...
      // 2. Maior tabela de faixa (AUTOCON PRIME/COD 7 > 6 > 5 > 4 > 3 > 2 > 1)
      ...
      if (aPrimeMatch && bPrimeMatch) {
        ...
        if (bNum !== aNum) return bNum - aNum; // PRIME 7 > PRIME 6 ... > PRIME 1
      }
