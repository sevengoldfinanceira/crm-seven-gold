...
function rankProposals(allOptions, params) {
  ...
  // use_half_installment now means the user explicitly chose INTEGRAL (100%).
  // Default behavior (when false/absent) is to use 50% of the installment.
  const useIntegralInst = params.use_half_installment === true || params.use_half_installment === 'true' || params.half_installment === true || params.half_installment === 'true';
  const useHalfInst = !useIntegralInst;
  ...
  allOptions.forEach((option) => {
    ...
    // Calculate installment to compare against user's max_installment limit:
    // Default (useHalfInst = true): compare against 50% of final installment (Meia Parcela).
    // When user chose integral (useIntegralInst = true): compare against 100% (full installment).
