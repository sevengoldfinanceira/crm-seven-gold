...
function rankProposals(allOptions, params) {
  const desiredCredit = parseBrCurrency(params.desired_credit || 0);
  ...
  const maxFirstInst = parseBrCurrency(params.maximum_first_installment || Infinity);
  const maxTempInst = parseBrCurrency(params.maximum_temporary_installment || Infinity);
  const maxFinalInst = parseBrCurrency(params.maximum_final_installment || Infinity);
  ...
  allOptions.forEach((option) => {
    ...
    const tempInst = Number(option.temporary_installment_value);
    ...
    const tempExcess = tempInst > maxTempInst ? tempInst - maxTempInst : 0;
    const finalExcess = finalInst > maxFinalInst ? finalInst - maxFinalInst : 0;
    ...
    const totalExcess = firstExcess + tempExcess + finalExcess;

    if (isCreditValid && totalExcess === 0) {
      // Perfectly within strict limits
      validProposals.push({
        ...option,
        credit_diff: Math.abs(credit - desiredCredit),
        temp_months: (option.temporary_installment_end - option.temporary_installment_start + 1) || 0,
      });
    } else if (isCreditValid && totalExcess > 0 && totalExcess <= (desiredCredit * 0.15 || 5000)) {
      ...
      if (tempExcess > 0) excessMessages.push(`Ultrapassa a parcela temporária em R$ ${tempExcess.toFixed(2).replace('.', ',')}`);
      if (finalExcess > 0) excessMessages.push(`Ultrapassa a parcela posterior em R$ ${finalExcess.toFixed(2).replace('.', ',')}`);
  ...
  if (priority === 'MAIOR_CREDITO') {
    ...
    validProposals.sort((a, b) => a.first_installment - b.first_installment || a.credit_diff - b.credit_diff);
  } else if (priority === 'MENOR_TEMPORARIA') {
    validProposals.sort((a, b) => a.temporary_installment_value - b.temporary_installment_value || a.first_installment - b.first_installment);
  } else if (priority === 'MENOR_POSTERIOR') {
    validProposals.sort((a, b) => a.final_installment_value - b.final_installment_value || a.first_installment - b.first_installment);
    ...
    validProposals.forEach(p => {
      const creditScore = (1 - (p.credit_diff / maxCreditDiff)) * RANKING_WEIGHTS.CREDIT_MATCH;
