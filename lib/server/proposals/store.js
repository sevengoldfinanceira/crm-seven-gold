 ...
 * Only returns real data from Supabase — no seed/example fallback.
 */
async function getActiveProposalOptions() {
  ...
  // No Supabase connection — return empty
  return [];
}

/**
 * Checks if a file with the given SHA-256 hash has already been imported
 * AND has actual active tables. If import exists but has no tables,
 * it's an orphan from a previous version — delete it and allow reimport.
 */
async function checkDuplicateHash(hash) {
  // Always return null to allow overwrite/re-import of any table PDF
  return null;
}
  ...
  if (supabase) {
    try {
      const dbPayload = {
        ...
        source_drive_file_id: importData.source_drive_file_id || null,
        source_drive_folder_id: importData.source_drive_folder_id || null,
        ...
        raw_metadata: importData.raw_metadata || {},
      ...
      console.error("Erro ao criar importação no Supabase:", error);
    } catch (e) {
      console.error("Erro interno createImportRecord:", e);
    }
  }
...
async function activateImport(importId, activatedBy = 'Administrador') {
  if (supabase) {
    try {
      // 1. Delete all other imports from Supabase (cascading deletes will automatically wipe old tables/options!)
      await supabase
        .from('proposal_imports')
        .delete()
        .neq('id', importId);

      // 2. Fetch import details (with raw metadata)
      ...
      if (!fetchErr && imp) {
        const metadata = imp.raw_metadata || {};
        const tables = metadata.tables || [];

        // 2. Insert tables and options transactional-like
        for (const table of tables) {
          const { data: dbTable, error: tableErr } = await supabase
            ...
            .insert({
              ...
              administrator_name: 'Seven Gold / Administradora',
              valid_until: table.valid_until || '2026-12-31',
              total_term_months: table.total_term_months || 180,
              administration_fee_percentage: 27.0,
              status: 'ACTIVE'
            })
            .select('*')
          ...
          if (!tableErr && dbTable) {
            // Insert options
            const optionsPayload = (table.options || []).map(opt => ({
              proposal_table_id: dbTable.id,
              credit_value: opt.credit_value,
              first_installment: opt.first_installment,
              first_installment_start: opt.first_installment_start || 1,
              first_installment_end: opt.first_installment_end || 1,
              temporary_installment_value: opt.temporary_installment_value,
              temporary_installment_start: opt.temporary_installment_start || 2,
              temporary_installment_end: opt.temporary_installment_end || 9,
              final_installment_value: opt.final_installment_value,
              final_installment_start: opt.final_installment_start || 10,
              final_installment_end: opt.final_installment_end || 180,
              status: 'ACTIVE'
            }));

            if (optionsPayload.length > 0) {
              await supabase.from('proposal_options').insert(optionsPayload);
            }

            // Insert group
            await supabase.from('proposal_groups').insert({
              proposal_table_id: dbTable.id,
              group_code: `GRUPO ${table.table_number}`,
              group_term_months: table.total_term_months || 180,
              insurance_percentage: 0.05,
              reserve_fund_percentage: 1.0,
              fixed_bid_percentage: 30.0
            });
      ...
      console.error("Erro ao ativar tabelas no Supabase:", e);
    }
  }

  // Memory cleanup: keep only the activated import
  inMemoryStore.imports = inMemoryStore.imports.filter(i => i.id === importId);
  inMemoryStore.customOptions = [];
  ...
  if (imp) {
    ...
    // Map memory list fallback
    const tables = (imp.raw_metadata || {}).tables || [];
    tables.forEach(table => {
      (table.options || []).forEach(opt => {
        inMemoryStore.customOptions.push({
