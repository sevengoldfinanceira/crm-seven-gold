...
async function checkDuplicateHash(hash) {
  if (supabase) {
    try {
      ...
      if (data) {
        // Check if this import actually has tables
        ...
        if (tables && tables.length > 0) {
          // Real duplicate — block
          return data;
        }

        // Orphan import with no tables — delete it and allow reimport
        await supabase.from('proposal_imports').delete().eq('id', data.id);
        return null;
      }
    } catch (e) {
      console.error("checkDuplicateHash error:", e);
    }
  }
...
async function activateImport(importId, activatedBy = 'Administrador') {
  if (supabase) {
    try {
      // 1. Fetch import details (with raw metadata)
