...
function parseProposalPdfText(pdfText, fileName = 'tabela_comercial.pdf') {
  ...
  blocks.forEach((block, idx) => {
    ...
    // 1. Try horizontal parsing (same line only using [^\d\r\n.,]+ instead of \s+)
    const rowRegex = /(?:R\$\s*)?(\d{1,3}(?:[.,\s]?\d{3})*[.,]\d{2})[^\d\r\n.,]+(?:R\$\s*)?(\d{1,3}(?:[.,\s]?\d{3})*[.,]\d{2})[^\d\r\n.,]+(?:R\$\s*)?(\d{1,3}(?:[.,\s]?\d{3})*[.,]\d{2})[^\d\r\n.,]+(?:R\$\s*)?(\d{1,3}(?:[.,\s]?\d{3})*[.,]\d{2})/g;
    let match;
    ...
    if (options.length === 0) {
      ...
      // Fallback: If split columns are not 4+, try flat-list sequential vertical parsing
      if (options.length === 0) {
        const allValues = [];
        let cMatch;
        currencyRegex.lastIndex = 0;
        while ((cMatch = currencyRegex.exec(block)) !== null) {
          allValues.push(parseBrCurrency(cMatch[1]));
        }

        if (allValues.length >= 4) {
          const N = Math.floor(allValues.length / 4);
          if (N > 0) {
            const tempOptions = [];
            let isValid = true;
            for (let i = 0; i < N; i++) {
              const credit = allValues[i];
              const first = allValues[N + i];
              const temp = allValues[2 * N + i];
              const final = allValues[3 * N + i];

              // Validation to guarantee we found the correct column sequence
              if (!(credit > first && first > temp)) {
                isValid = false;
                break;
              }
            ...
            if (isValid && tempOptions.length > 0) {
