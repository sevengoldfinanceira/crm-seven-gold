...
function parseProposalPdfText(pdfText, fileName = 'tabela_comercial.pdf') {
  ...
  targetBlocks.forEach((block, idx) => {
    ...
    // Clean footer texts, author names, and dates to avoid concatenated year digits in currency numbers
    const cleanBlock = block
      .replace(/(?:PAULO SANTOS|Camila Martins|ADMINISTRADORA DE CONSORCIO)[^\n\r]*/gi, '')
    ...
    const values = [];
    let cMatch;
    while ((cMatch = currencyRegex.exec(cleanBlock)) !== null) {
    ...
    for (let i = 0; i + 3 < values.length; i += 4) {
      ...
      // Validate relationship: Credit > 1st Inst and 1st Inst >= Temp Inst, and Sanity Max Credit (2M)
      if (credit > first && first >= temp && temp > 0 && credit <= 2000000) {
        options.push({
          credit_value: credit,
