...
async function syncGoogleDriveUrl(driveUrlInput, options = {}) {
  ...
  try {
    ...
    if (processedFilesCount === 0 || combinedTables.length === 0) {
      return {
        success: false,
        message: 'Não foi possível extrair planos ou propostas comerciais dos arquivos do Google Drive. Verifique o formato dos arquivos.',
      };
    }
    ...
    const importRecord = await createImportRecord({
      ...
      source_file_name: target.type === 'FOLDER'
        ? `Pasta Google Drive (${processedFilesCount} arquivo${processedFilesCount > 1 ? 's' : ''})`
    ...
    return {
      ...
      file_id: target.id,
      drive_url: targetUrl,
      message: `Google Drive sincronizado com sucesso! ${combinedTables.length} planos e ${totalProposalRows} propostas foram importados (${processedFilesCount} PDF${processedFilesCount > 1 ? 's' : ''} lido${processedFilesCount > 1 ? 's' : ''}).`,
