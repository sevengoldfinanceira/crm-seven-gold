...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const levelId = mapCargoToCommissionLevelId(seller.cargo);
    if (!levelId) {
      return send(res, 400, {
        ok: false,
        error: 'O cargo deste colaborador não possui uma matriz de comissão configurada.',
      });
    }
    const items = [];
    ...
    for (const sale of sales) {
      ...
      const percentage = parseCommissionRate(valueStr);
