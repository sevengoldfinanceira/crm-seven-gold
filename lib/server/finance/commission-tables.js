...
const { getAuthorizedCrmUser, normalizeEmail } = require('../crm-authorization');
const { mapCargoToCommissionLevelId, parseCommissionRate } = require('./commission-rules');

const MASTER_COMMISSION_EMAIL = 'sevengoldfinanceira@gmail.com';
const MASTER_SELECTABLE_LEVEL_IDS = new Set([
  'home-office',
  'assistente-vendas',
  'consultor-vendas',
  'coordenador',
...
const getMasterOptions = async () => {
  ...
  const levels = Array.from(levelsById.values())
    .filter((level) => MASTER_SELECTABLE_LEVEL_IDS.has(level.id))
    .sort((a, b) => {
...
module.exports = async (req, res) => {
  ...
  try {
    const isMaster = normalizeEmail(auth.user?.email) === MASTER_COMMISSION_EMAIL;
    ...
    if (isMaster) {
      ...
      if (targetUserId) {
        ...
        if (!level || !MASTER_SELECTABLE_LEVEL_IDS.has(level.id)) {
