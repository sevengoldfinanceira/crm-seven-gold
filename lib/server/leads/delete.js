...
const { getAuthorizedCrmUser, canAccessLead } = require('../crm-authorization');
const { assertLeadMutable } = require('../commercial-productions');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const requiredCleanup = [
      ...
      { table: 'lead_activity_logs', label: 'histórico de atividades' },
      { table: 'lead_history', label: 'histórico' },
      { table: 'crm_stage_events', label: 'histórico de estágios' },
      { table: 'crm_appointments', label: 'agendamentos crm' },
    ];
    for (const dependency of requiredCleanup) {
      const { error: cleanupError } = await supabase.from(dependency.table).delete().in('lead_id', ids);
      if (cleanupError) {
        const msg = String(cleanupError.message || "").toLowerCase();
        const code = String(cleanupError.code || "");
        const isTableNotFound = code === "PGRST301" || code === "42P01" || msg.includes("schema cache") || msg.includes("could not find the table") || msg.includes("does not exist") || msg.includes("relation");
        if (!isTableNotFound) {
          return send(res, 409, {
            ok: false,
            error: `Não foi possível remover ${dependency.label} do lead: ${cleanupError.message}`,
          });
        }
      }
    }
    ...
    if (deleteHistory) {
      for (const table of ['messages', 'sales', 'tasks']) {
        const { error: cleanupError } = await supabase.from(table).delete().in('lead_id', ids);
        if (cleanupError) {
          const msg = String(cleanupError.message || "").toLowerCase();
          const code = String(cleanupError.code || "");
          const isTableNotFound = code === "PGRST301" || code === "42P01" || msg.includes("schema cache") || msg.includes("could not find the table") || msg.includes("does not exist") || msg.includes("relation");
          if (!isTableNotFound) {
            return send(res, 409, {
              ok: false,
              error: `Não foi possível remover os registros relacionados em ${table}: ${cleanupError.message}`,
            });
          }
        }
      }
