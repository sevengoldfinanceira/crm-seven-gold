...
async function getLeadProduction(leadId) {
  const { data, error } = await supabase.from('leads').select('id,production_id,commercial_productions!production_id(id,name,status,month,year)').eq('id', leadId).maybeSingle();
...
const isProductionSchemaError = (error) => {
  const text = `${error?.code || ''} ${error?.message || ''} ${error?.details || ''}`.toLowerCase();
  return ['pgrst204', 'pgrst205'].includes(String(error?.code || '').toLowerCase())
    || /production_id|production_month|production_year|commercial_productions|schema cache|relationship/.test(text);
};

const stripProductionFields = (payload) => {
  const clean = { ...(payload || {}) };
  [
    'production_id',
    'production_month',
...
module.exports = { NO_OPEN_PRODUCTION, CLOSED_PRODUCTION, isDirectorCeo, getOpenProduction, getProduction, getLeadProduction, assertLeadMutable, productionFields, isProductionSchemaError, stripProductionFields };
